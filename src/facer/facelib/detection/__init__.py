...
from facer.facelib.utils import load_file_from_url
from facer.facelib.utils import download_pretrained_models
from facer.facelib.detection.yolov5face.models.common import Conv
...
def init_retinaface_model(model_name, half=False, device='cuda'):
    if model_name == 'retinaface_resnet50':
        ...
        raise NotImplementedError(f'{model_name} is not implemented.')
    if not os.path.isfile("weights/detection_Resnet50_Final.pth"):
        try:
            response=requests.get(model_url)
            if response.status_code==200:
                with open("weights/detection_Resnet50_Final.pth",'wb') as file:
                    file.write(response.content)
                print("detection_Resnet50_Final.pth downloaded successfully")
            else:
                print("fail to download detection_Resnet50_Final.pth")
        except:
