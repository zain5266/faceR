...
if __name__ == '__main__':
    ...
    if not os.path.isfile("weights/codeformer_colorization.pth"):
        try:
            url="https://github.com/sczhou/CodeFormer/releases/download/v0.1.0/codeformer_colorization.pth"
            response=requests.get(url)
            if response.status_code==200:
                with open("weights/codeformer_colorization.pth",'wb') as file:
                    file.write(response.content)
                print("codeformer_colorization.pth downloaded successfully")
            else:
                print("fail to download codeformer_colorization.pth")
        except:
            print("Error occure while downloading codeformer_colorization.pth ")    
    device = get_device()
    ...
    # ckpt_path = load_file_from_url(url="weights/codeformer_colorization.pth", 
    #                                 model_dir='weights/CodeFormer', progress=True, file_name=None)
    ckpt_path="weights/codeformer_colorization.pth"
